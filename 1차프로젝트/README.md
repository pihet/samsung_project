...
### 1. 🔍 다나와 실시간 웹 크롤링 및 규격/단가 파싱 (`collector.py`)
- **100% 다나와(Danawa) 실시간 크롤링 연동**: 다나와 정식 상품 코드(`pcode`), 실시간 최저가, 판매처, 대표 이미지, 상세 스펙 태그 자동 수집.
...
- **보안 회원가입/로그인 모달**: `bcrypt` 단방향 해시 암호화를 적용한 안전한 사용자 인증 및 F5 새로고침 세션 유지.
- **회원 전용 찜하기 & 목표 알림가 설정**: 관심 상품 찜 목록 DB 저장 및 목표가 수정 지원.

### 3. 📧 Resend API 하이브리드 자동 이메일 알림 엔진 (`db_manager.py`)
- **목표가 달성 이메일 자동 발송**: 찜한 상품의 실시간 최저가가 목표가 이하로 하락(`lprice ≤ target_price`) 시 자동 탐지.
- **Resend API 연동 (개인 비밀번호 불필요)**: 개인 비밀번호 설정 없이 Resend API Key로 수신자 네이버/구글 메일에 고품질 HTML 알림 메일 즉시 발송.
- **중복 알림 방지 & 상태 리셋**: PostgreSQL `is_alert_sent` 상태로 중복 메일 차단 및 목표가 변경 시 알림 플래그 자동 리셋.

### 4. 📊 가격 추세 다중 비교 UX (`app.py` - Tab 3)
...
### 5. 🤖 AI 14일 미래 가격 예측 & 세일 캘린더 연동 (`ml_forecaster.py` & `analyzer.py`)
...
- **쇼핑몰 세일 임박 알림 전용 카드**: 세일 14일 전 바짝 다가온 세일 이벤트를 감지하여 구매 보류 알림 카드 제공.

### 6. 💡 차별화 킬러 기능 (`analyzer.py`)
...
| **Data Collection** | **Danawa Real-time Web Crawler (`collector.py`)**, Requests, BeautifulSoup4, Regex Parser |
| **Email Notification** | **Resend Email API (`resend.com`)**, smtplib (Fallback) |
...
### 2. `user_favorites` / `favorites` (찜 목록 및 목표가 알림)
```sql
...
### 2. 환경 변수 설정 (`.env`)
`.env.example` 파일을 복사하여 프로젝트 루트에 `.env` 파일을 생성하고 DB 비밀번호 및 Resend 이메일 API 키를 입력합니다.
```env
DB_PASS=1111
