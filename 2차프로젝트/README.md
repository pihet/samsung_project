...
2. **초고속 실시간 AI 디스패칭 (Action-Masked PPO)**: 정반 208차원 상태 벡터와 유효 행동 마스킹을 결합하여 밀리초 단위($\approx 1.8\text{ ms}$) 즉시 추천.
...
## 3. 4대 핵심 물리/시간 제약 조건 (Hard Constraints)

모든 알고리즘은 `modeling/eval_metrics.py` 엔진을 통해 **제약 위반 0건 (100% Feasible)**을 전수 감사받습니다:
...
### 1) 통합 시뮬레이터 (Unified Sequential Simulator) 실측 결과
...
| :---: | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| 1 | **Google OR-Tools CP-SAT (Deep)** | 수리 최적화 (8-Core 병렬) | **1,210일** | **246개 (28.2%)** | **50.5일** | **29.4%** | **0건** | **PASS** | 18.92초 |
| 2 | **Google OR-Tools CP-SAT (Det)** | 수리 최적화 (결정론적 단일) | **1,254일** | **248개 (28.4%)** | **55.8일** | **28.4%** | **0건** | **PASS** | 17.20초 |
| 3 | **EST Heuristic (Unified Sim)** | 규칙 기반 휴리스틱 | **1,254일** | **248개 (28.4%)** | **55.8일** | **28.4%** | **0건** | **PASS** | **0.19초** |
...
### 2) 기존 학술 연구 논문 베이스라인 (Figure 10 2D Packing Reference)
...
### 1) PPO Ablation Study (3-Seed: `42, 100, 2024`)
...
## 8. 프로젝트 디렉토리 구조 및 아티팩트 관리

본 프로젝트는 중앙집중식 경로 관리 모듈(`utils/paths.py`)을 통해 모든 아티팩트를 5대 서브폴더로 체계적으로 분리·보존합니다:
